...
<div class="my-3 p-3 bg-body rounded ">
    ...
    <div class="d-flex text-body-secondary pt-1">
        <textarea id="longparagraf-1" name="research_title" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;"
            rows="3"
            placeholder="Başlıkta karşılaştırdığınız olgular veya incelediğiniz konu anlaşılabilir düzeyde yer almalıdır. | The facts you are comparing or the subject you are examining should be included in the title at an understandable level.">{{ trim($formData ? $formData->research_title : old('research_title')) }}</textarea>
    </div>
...
Why is this research important? What distinguishes this research from others? Mention the contributions of this research to the literature in short and clear sentences. Please indicate if it will make a very special contribution to the world of science or humanity other than literature.">{{ trim($formData ? $formData->research_unique_value : old('research_unique_value')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Hipotezler / Araştırma Soruları | Hypothesis / Research
        Questions</label>
    <div class="d-flex text-body-secondary pt-1">
...
Write the research questions or hypotheses you have determined according to the nature of the research.">{{ trim($formData ? $formData->research_hypothesis : old('research_hypothesis')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Yöntem | Method</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea name="research_method" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="6"
...
While writing this part, you need to clearly write which research method you chose, the scientific paradigm on which the research method is based, which technique the method requires, which technique you will use in your study, and why you chose this technique.">{{ trim($formData ? $formData->research_method : old('research_method')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Evren ve Örneklem | Universe and Sample</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="4" name="research_universe"
...
Write the population of the research and how you chose the sample from this population. Please write which type of sampling you used.">{{ trim($formData ? $formData->research_universe : old('research_universe')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Ölçek ve Formlar | Scale and Forms</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea name="research_forms" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="4"
            placeholder="Araştırmada kullanılacak ölçek ve formların kaynağını yazınız.
Write the source of the scales and forms to be used in the research.">{{ trim($formData ? $formData->research_forms : old('research_forms')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Verilerin Toplanması ve Analizi | Data Collection and
        Analysis</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea name="research_data_collection" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="4"
            placeholder="Araştırmada kullanılacak olan her ölçme aracına yönelik detaylı bilgi ayrı başlıklar halinde verilmelidir.
Detailed information about each measurement tool to be used in the research should be given under separate headings.">{{ trim($formData ? $formData->research_data_collection : old('research_data_collection')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Sınırlar ve Kısıtlar | Boundaries and Restrictions</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea name="research_restrictions" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="4"
...
How did you delimit the scope and topic of your research and what are the main constraints on applying the research?">{{ trim($formData ? $formData->research_restrictions : old('research_restrictions')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Araştırma Tarih ve Yeri | Research Date and Place</label>
    <div class="d-flex text-body-secondary pt-1">
        <textarea name="research_place_date" class="form-control pt-0 pb-0 mb-2" style=" overflow: hidden;" rows="4"
...
Create a research calendar for when and where you will conduct field work (if any).">{{ trim($formData ? $formData->research_place_date : old('research_place_date')) }}</textarea>
    </div>
    <label class="p-2 badge rounded-pill bg-primary mb-0 ">Faydalanılacak Kaynaklar/Literatür Taraması | Sources to
