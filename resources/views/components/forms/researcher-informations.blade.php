<div class="container pt-1">
    <div class="row g-3">
        <div class="col-md-4">
            <label for="researcherName" class="form-label">Araştırmacı Adı <br><span class="pt-0 small">Name</span></label>
            <input type="text" name="name" id="name" class="form-control" id="researcherName">
        </div>
        <div class="col-md-4">
            <label for="researcherName" class="form-label">Araştırmacı Soyadı <br><span
                    class="pt-0 small">Lastname</span></label>
            <input type="text" name="lastname" class="form-control" id="researcherName">
        </div>
        <div class="col-md-4">
            <label for="studentID" class="form-label">Öğrenci No / TC No<br><span class="pt-0 small">Student ID
                    Number</span></label>
            <input type="text" class="form-control" id="studentID" placeholder="">
        </div>
        <div class="col-md-4">
            <label for="email" class="form-label">Mail Adressi<br><span class="pt-0 small">Email
                    Address</span></label>
            <input type="email" class="form-control" id="email">
        </div>
        <div class="col-md-4">
            <label for="phone" class="form-label">Telefon<br><span class="pt-0 small">Phone
                    Number</span></label>
            <input type="phone" class="form-control" id="phone">
        </div>
        <div class="col-md-4">
            <label for="advisor" class="form-label">Danışman / Yürütücü<br><span
                    class="pt-0 small">Advisor/Executive</span></label>
            <input type="text" class="form-control " id="advisor">
        </div>
        <div class="col-md-4">
            <label for="department" class="form-label">Anabilim Dalı<br><span
                    class="pt-0 small">Department</span></label>
            <input type="text" class="form-control " id="department" placeholder="">
        </div>
        <div class="col-md-4">
            <label for="program" class="form-label">Program<br>
                <span class="pt-0 small">Department</span></label>
            <input type="text" class="form-control" id="program">
        </div>
    </div>
</div>
